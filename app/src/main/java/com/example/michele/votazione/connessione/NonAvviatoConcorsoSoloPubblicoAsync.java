...
public class NonAvviatoConcorsoSoloPubblicoAsync extends AsyncTask<String, Void, String> {

    private ProgressDialog loadingDialog;
    private Context context;
    private static String progetto;

    public NonAvviatoConcorsoSoloPubblicoAsync(Context context){
    ...
    protected String doInBackground(String... params) {
        ...
        if(isNetworkAvailable(context)){
            ...
            try{
                ...
                Log.d("sdfd","aaaa"+progetto);

                // Dati inoltrati al server
                String data = "?progetto=" + URLEncoder.encode(progetto, "UTF-8");

                // Link
                String link ="http://databaseandroid.altervista.org/app/Control/NonAvviatoConcorsoSoloPubblicoControl.php" + data;
                Log.d("DEBUG-------:", link);
    ...
    protected void onPostExecute(String result) {
        ...
        if (result != null) {
            ...
            if(bool){
                Toast.makeText(context, "Dati registrati correttamente correttamente", Toast.LENGTH_SHORT).show();
                Intent intent = new Intent(context, HomeOrganizzatore.class);
                intent.putExtra("Numero", (Serializable) 1);
                context.startActivity(intent);
            } else {
                Toast.makeText(context, "Errore: Dati non registrato riprovare", Toast.LENGTH_SHORT).show();
            }
