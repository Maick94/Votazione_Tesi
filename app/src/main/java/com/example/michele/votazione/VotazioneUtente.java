...
import com.example.michele.votazione.adapters.AdapterVotazioneUtente;
...
import com.example.michele.votazione.connessione.AvviaVotazioneUtenteAsync;
...
public class VotazioneUtente extends AppCompatActivity {
    ...
    private AdapterVotazioneUtente adapter;
    ...
    private static int[] dati;
    private Progetti progetti=null;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        try {
            resultGson = new AvviaVotazioneUtenteAsync(VotazioneUtente.this).execute(progetto).get();
        ...
        adapter = new AdapterVotazioneUtente(models, this);
        ...
        button.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                Votazione votazione=new Votazione(progetti.getProgettoList().get(index).getIdConcorso(), progetti.getProgettoList().get(index).getId(), "Pubblico",""+voto);
                index++;
                String insV = new Gson().toJson(votazione, Votazione.class);
                new InserimentoVotazioneAsync(VotazioneUtente.this).execute(insV);
                if(viewPager.getCurrentItem() == models.size()-1) {
                    Utente u=new Utente(imei,idConcorso);
                    String insU = new Gson().toJson(u, Utente.class);
                    new InserimentoUtenteAsync(VotazioneUtente.this).execute(insU);
                    button.setText("Fine Votazione");
                    Intent intent = new Intent(VotazioneUtente.this, MainActivity.class);
