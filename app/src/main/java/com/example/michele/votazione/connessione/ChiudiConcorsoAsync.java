...
public class ChiudiConcorsoAsync extends AsyncTask<String, Void, String> {

    private ProgressDialog loadingDialog;
    private Context context;
    private static String chiudi;

    public ChiudiConcorsoAsync(Context context){
    ...
    protected String doInBackground(String... params) {
        ...
        if(isNetworkAvailable(context)){
            ...
            try{
                // Parametro  passati da execute
                chiudi = params[0];


                // Dati nickname e password inoltrati al server
                String data = "?chiudi=" + URLEncoder.encode(chiudi, "UTF-8");



                // Link
                String link = "http://databaseandroid.altervista.org/app/Control/ChiudiConcorsoControl.php"+data;
                Log.d("DEBUG:", link);
    ...
    protected void onPostExecute(String result) {
        ...
        if (result != null) {
            ...
            if(bool){
                Toast.makeText(context, "Concorso chiuso correttamente", Toast.LENGTH_SHORT).show();
                Intent intent = new Intent(context, HomeOrganizzatore.class);
                intent.putExtra("Numero", (Serializable) 1);
                context.startActivity(intent);
            } else {
                Toast.makeText(context, "Errore: Impossibile chiudere concorso", Toast.LENGTH_SHORT).show();
            }
